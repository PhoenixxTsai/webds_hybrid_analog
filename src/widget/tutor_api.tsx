...
export async function SendGetSetting(module: any): Promise<any> {
  const dataToSend = {
    task: "getSetting"
  };

  try {
    let reply = await requestAPI<any>(`tutor/${module}`, {
      body: JSON.stringify(dataToSend),
      method: "POST"
    });
    return Promise.resolve(reply);
    ...
    console.log(error);
    return Promise.reject(`getSetting failed: ${error.toString()}`);
  }
}

export async function SendGetADC(module: any): Promise<any> {
  const dataToSend = {
    task: "getAdcRange"
  };

  try {
    let reply = await requestAPI<string>(`tutor/${module}`, {
      body: JSON.stringify(dataToSend),
      method: "POST"
    });
    return Promise.resolve(reply);
    ...
    console.log(error);
    return Promise.reject(`getAdcRange failed: ${error.toString()}`);
  }
}
